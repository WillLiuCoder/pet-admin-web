...
import { getCaregiverById, updateCaregiver } from '@/api/caregiver'
  ...
  try {
    ...
    Object.keys(form).forEach(key => {
      if (data[key] !== undefined) {
        if (key === 'serviceZones' || key === 'certifications') {
          try {
            form[key] = typeof data[key] === 'string' ? JSON.parse(data[key]) : data[key] || []
          } catch {
            form[key] = []
          }
        } else {
